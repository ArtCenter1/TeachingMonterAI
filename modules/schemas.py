...
class StudentModel(BaseModel):
    ...
    misconception_risk: Dict[str, List[str]] = Field(
        default_factory=dict, description="Top-3 likely errors by concept"
    )
    cognitive_load_budget: float = Field(..., description="Max concept density ceiling")
    modality_preference: ModalityPreference = Field(
        ..., description="Preferred learning modality"
    )
    abstraction_tolerance: float = Field(
        ..., description="0=concrete only, 1=fully abstract"
    )
    ...
    def map_legacy_fields(cls, data: Any) -> Any:
        if isinstance(data, dict):
            ...
            for alt in ["topic", "requirement"]:
                if alt in data and (
                    "course_requirement" not in data or not data["course_requirement"]
                ):
            ...
            for alt in ["persona", "student_model"]:
                if alt in data and (
                    "student_persona" not in data or not data["student_persona"]
                ):
                    val = data[alt]
...
class AIStudentFeedback(BaseModel):
    run_id: str = Field(
        ..., description="The run ID from the original generation request"
    )
    ai_student_scores: Dict[str, Any] = Field(
        ..., description="Structured scores from the AI Student evaluator"
    )
