...
class ConceptPlanner:
    ...
    def _build_prompt(self, topic: str, student_model: StudentModel, retry_hint: str = "") -> str:
        """
...
List every sub-concept a student must understand BEFORE grasping "{topic}".
Order them from simplest (assumed knowledge) to most complex (the target topic).

STEP 2 — OUTPUT the concept graph as JSON.

CONSTRAINTS (strictly enforced):
- You MUST return between {MIN_CONCEPT_NODES} and 7 concept nodes.
    ...
    async def plan(self, topic: str, student_model: StudentModel, model_override: str = None) -> ConceptGraph:
        ...
        for attempt in range(1, 3):  # Max 2 attempts
            ...
            try:
                ...
                logger.debug(f"M3 [{topic}] — attempt {attempt}: generated {node_count} node(s)")
                ...
                if node_count < MIN_CONCEPT_NODES:
                    ...
                    logger.warning(f"M3 under-generation on attempt {attempt}: {last_error}")
                ...
                logger.info(f"M3 [{topic}] — successfully generated {node_count} nodes on attempt {attempt}")
                return graph

            except Exception as e:
                last_error = str(e)
                logger.warning(f"M3 attempt {attempt} failed with exception: {last_error}")
                continue
        ...
        return ConceptGraph(
            nodes=[
                ...
                ConceptNode(
                    ...
                    misconceptions=[f"Assuming {topic} applies universally without conditions"],
