...
class VideoRenderer:
    ...
    async def render(
        ...
        for i, segment in enumerate(script.segments):
            ...
            try:
                # Use SSE streaming for TTS with timestamps per Cartesia SDK v3.x
                stream = self.client.tts.generate_sse(
                    model_id="sonic-3",
                ...
                for event in stream:
                    if event.type == "chunk" and event.audio:
                        segment_audio_data += event.audio

                    elif event.type == "timestamps":
                        ts_obj = event.word_timestamps
                        if ts_obj:
                            words = getattr(ts_obj, "words", [])
                            starts = getattr(
                                ts_obj, "start", getattr(ts_obj, "starts", [])
                            )
                        ...
                        logger.error(
                            f"Cartesia stream error on {segment.segment_id}: {event.error}"
                        )
                        break
                ...
                logger.exception(
                    f"Audio generation failed for {segment.segment_id}: {str(e)}"
                )
            ...
            if returncode == 0:
                ...
                logger.error(
                    f"FFmpeg failed for segment {segment.segment_id}: {stderr.decode() if stderr else 'unknown error'}"
                )
        ...
        def run_concat():
            return subprocess.run(
                concat_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False
            )
    ...
    def _generate_srt(self, timestamps: List[Dict[str, Any]], output_path: str):
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            for i in range(0, len(timestamps), block_size):
                ...
                f.write(
                    f"{format_time(chunk[0]['start'])} --> {format_time(chunk[-1]['end'])}\n"
                )
