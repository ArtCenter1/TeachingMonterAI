...
- If `execute_task` times out, first check the host's MCP timeout settings. Changing from `stdio` to `streamable-http` does not remove host-side per-call time limits.
