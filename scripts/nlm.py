...
async def cmd_login(args):
    ...
    import subprocess
    import sys
    # Use the same Scripts directory as the current python interpreter
    python_dir = os.path.dirname(sys.executable)
    notebooklm_cmd = os.path.join(python_dir, "notebooklm")
    if sys.platform == "win32" and not notebooklm_cmd.endswith(".exe"):
        notebooklm_cmd += ".exe"
    
    if os.path.exists(notebooklm_cmd):
        subprocess.run([notebooklm_cmd, "login"], check=True)
    else:
        # Fallback to PATH
